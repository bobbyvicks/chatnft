...
/* Built by pixelfixer/tools/build.js from pixelfixer/src. PF_BUNDLE points
   somewhere else only when trying a rebuild before it is committed. */
const path = require('path');
const BUNDLE = process.env.PF_BUNDLE
  || path.join(path.dirname(FILE), 'pixelfixer', 'pixelfixer.bundle.js');
...
swap('<section class="panelbox pg-agent">', block([
  ...
  '  <p class="note">This is their quick pass: three detectors, and the answer they agree on.',
  '    The thorough pass that arbitrates when those three disagree is not built yet, so a',
  '    hard image can come back with a grid it is unsure of - it says which.</p>',
  ...
  '  <div class="agjob">',
  /* ONE MODE, BECAUSE ONE MODE WORKS. The reference has a "full" pass that
     arbitrates between the detectors when they disagree; it is about 3,300
     lines that are not ported, and core.detect throws on it by name rather
     than quietly answering with something else. Offering it here as the
     DEFAULT - which the first draft did - made the main button on the page
     throw on its first press. It comes back as a second option when it is
     built; until then the page says so rather than implying otherwise. */
  '    <label for="fixmode">Mode</label>',
  '    <select id="fixmode" title="How hard to look for the grid. The thorough pass is not built yet."><option value="fast">Quick - three detectors</option></select>',
...
/* THE BUTTON, not the string: the eight pairwise CSS rules that hide the
   page carry data-page="fixer" too, and they are in the head with it. */
if (markup.split('class="pgtab" data-page="fixer"').length !== 2)
...
/* THE DEFAULT MODE IS ONE THE ENGINE ANSWERS TO. A menu whose first entry
   throws is a button that fails on its first press, which is exactly how
   this read in its first draft. */
{
  const a = markup.indexOf('<select id="fixmode"');
  const sel = markup.slice(a, markup.indexOf('</select>', a));
  const first = /value="([a-z]+)"/.exec(sel);
  if (!first || first[1] !== 'fast')
    throw new Error('the default mode is "' + (first && first[1]) + '", which detect() refuses');
  if (sel.indexOf('value="full"') >= 0) throw new Error('full mode is offered and is not ported');
}
