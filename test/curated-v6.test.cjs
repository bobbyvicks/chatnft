...
const bundle=JSON.parse(read('rules/strict-fit-v6-collection.json')),canonical=JSON.parse(read('rules/trait-rules-strict-fit-v6.json'));
...
test('v6 bundled data enforces the reviewed exclusions in 10000 native Pixelbench draws',()=>{
 const c=setup(),p=c.planRuleImport(bundle.rules,traits);assert.equal(p.actions,427);
...
function controls(c,items){
 ...
 vm.runInContext(read('curated-rules-v6.js'),c);return els;
}
test('Load v6 button passes the full review to the native importer and saves the correct paint order',async()=>{
 const c=setup(),els=controls(c,traits);c.LAYERS=bundle.order.slice().reverse();await els.rulev6.onclick();
 assert.deepEqual(c.imported,bundle.rules);assert.deepEqual(c.savedOrder,bundle.order);assert.equal(els.rulev6.disabled,false);assert.match(els.ruleimportnote.textContent,/V6 draw order saved/);
});
test('Load v6 refuses a missing inventory and a failed fetch before changing any rules',async()=>{
 const c=setup(),els=controls(c,traits.slice(1));await els.rulev6.onclick();assert.equal(c.imported,undefined);assert.match(els.ruleimportnote.textContent,/Missing 1/);
 c.fetch=async()=>({ok:false});await els.rulev6.onclick();assert.equal(c.imported,undefined);assert.equal(c.savedOrder,undefined);assert.match(els.ruleimportnote.textContent,/Could not load/);
});
